...
| **Provider** | The remote model API you point Hermes at, for example OpenRouter, Anthropic, OpenAI, Gemini, DeepSeek, or a custom endpoint. |
| **Model** | The specific LLM you use through that provider, for example `deepseek/deepseek-v4-flash:free`. |
| **API key** | The secret token your provider gives you. HADES stores it in `~/.hades/.env`. |
...
| **Workspace** | The directory at `~/.hades/workspace/` that Hermes can read and write. Bind-mounted into the container. |
...
| **Non-interactive install** | Running the installer without prompts, useful for CI or scripted use. Triggered with `HERMES_NONINTERACTIVE=1`. |
